"""Экстрактор транспорта мира книги."""
import json
import logging
...
log = logging.getLogger("hermes.world_extraction.transport")
KNOWLEDGE_DIR = Path(__file__).resolve().parent.parent.parent.parent / "KNOWLEDGE"


def extract_transport() -> ExtractionResult:
    """Извлечь данные о транспорте."""
    items = []
    source_files = []
            ...
            try:
                ...
                if isinstance(data, list):
                    for item in data:
                        ...
                        transport_keywords = ["путешеств", "дорог", "кон", "повоз", 
                                             "лодк", "корабл", "переезд", "маршрут"]
                        if any(kw in content.lower() for kw in transport_keywords):
                            items.append(WorldKnowledge(
                                id=f"transport_{topic.lower().replace(' ', '_')}",
                                name=topic,
                                name_ru=topic,
                                category="transport",
                ...
                log.error("transport_extract_error file=%s: %s", filename, e)
