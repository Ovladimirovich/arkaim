-- 000_baseline: Снимок текущего состояния readers.db
-- Дата: 2026-07-10
-- Назначение: зафиксировать текущую схему перед инкрементальными миграциями
-- Применяется ТОЛЬКО если таблицы уже существуют (CREATE IF NOT EXISTS)
...
-- Фиксируем что baseline применена
INSERT OR IGNORE INTO _migrations (name, applied_at) VALUES ('000_baseline.sql', datetime('now'));
